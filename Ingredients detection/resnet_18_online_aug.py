...
import skimage
import csv
from albumentations import Compose, Equalize, Rotate, OneOf, HueSaturationValue, Blur, ElasticTransform, RandomBrightness, RandomContrast, MotionBlur, GaussianBlur, GaussNoise, RandomBrightnessContrast
import albumentations as A


#load dataclass
from data_class_online_aug import IngredientsDatasetAug
...
#uncomment in case of need to augment bounding box
#bb_params = A.BboxParams(format='albumentations', label_fields = ['ingr'])
...
def aug_func(p=0.7):
    return Compose([
            ...
            #Rotate(limit = 10, border_mode = 0, value = (255,255,255), p = p),
            GaussNoise(var_limit = (2,10), p=0.6)
            ], p=p) #bbox_params = bb_params, p=p)

data_augmentation = {'train': aug_func(), 'val': None}


#Load datasets for train and val
img_datasets = {x: IngredientsDatasetAug(csv_file.format(x), os.path.join(img_folder,x), data_transforms[x], data_augmentation[x]) for x in ['train','val']}
